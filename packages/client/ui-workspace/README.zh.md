...
浏览区域顶部设有醒目的**新会话**入口——宽幅状态下是通栏主色按钮，轨道状态下是 36px 主色圆形控件——先清除当前搜索过滤，再复用注入的 `startSession` 流程，让新对话落在浏览树上；无会话空状态会以文案指明该入口。Session 行按日历感知梯度标注最后活动时间——刚刚、N 分钟前／N 小时前、昨天 HH:mm、再早的 MM-DD 与 YYYY-MM-DD——悬浮卡片复用同一标签。Workspace 分组头显示可见会话数（悬停时切换为操作按钮簇），包含当前所选会话的分组无论折叠与否都保持文件夹主色与标题加粗。组内 Session 行相对分组头缩进一级，单列表则恢复共享行内边距。各行均为可键盘触达的树节点——Enter 或空格即可激活——并带内嵌焦点环与按压、悬停背景过渡。
