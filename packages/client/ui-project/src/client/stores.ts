 ...
 * selection, load status, and the hidden-row filter. Levels are re-listed on
 * demand and deliberately not persisted — stale paths from a previous
 * session's workspace would only mislead. Module level exports the factory
 * only (a module-level handle would pin the store identity across plugin
 * reloads); register() receives the factory and the tree derives its
